...
export default function Urgencias() {
  ...
  useEffect(() => {
    const fetchCalidad = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/documentos/urgencias/listararchivos', {
          method: 'GET',
          headers: {
        ...
        if (data && data.listarUrgencias) {
          setlistarPDF(data.listarUrgencias);
          setListarDocumentos(data.listarUrgencias);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/documentos/urgencias/guardararchivos', {
        method: 'POST',
        body: formDataToSend,
      });
